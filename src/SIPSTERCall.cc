...
NAN_METHOD(SIPSTERCall::New) {
  ...
  call->emit = new Nan::Callback(
    Local<Function>::Cast(Nan::Get(call->handle(), Nan::New(emit_symbol)).ToLocalChecked())
  );
...
NAN_METHOD(SIPSTERCall::Answer) {
  ...
  if (info.Length() > 0 && info[0]->IsUint32()) {
    prm.statusCode = static_cast<pjsip_status_code>(info[0]->Int32Value(Nan::GetCurrentContext()).FromJust());
...
NAN_METHOD(SIPSTERCall::GetStats) {
  ...
  if (info.Length() > 0 && info[0]->IsBoolean()) {
    with_media = Nan::To<bool>(info[0]).ToChecked();
    if (info.Length() > 1 && info[1]->IsString()) {
