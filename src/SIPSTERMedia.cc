...
NAN_METHOD(SIPSTERMedia::New) {
  ...
  med->emit = new Nan::Callback(
    Local<Function>::Cast(Nan::Get(med->handle(), Nan::New(emit_symbol)).ToLocalChecked())
  );
...
NAN_METHOD(SIPSTERMedia::AdjustRxLevel) {
  ...
  if (med->media) {
    if (info.Length() > 0 && info[0]->IsNumber()) {
      try {
        med->media->adjustRxLevel(static_cast<float>(Nan::To<double>(info[0]).ToChecked()));
      } catch(Error& err) {
...
NAN_METHOD(SIPSTERMedia::AdjustTxLevel) {
  ...
  if (med->media) {
    if (info.Length() > 0 && info[0]->IsNumber()) {
      try {
        med->media->adjustTxLevel(static_cast<float>(Nan::To<double>(info[0]).ToChecked()));
      } catch(Error& err) {
