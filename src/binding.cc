...
#undef X
        ...
        call_obj = Nan::NewInstance(Nan::GetFunction(Nan::New(SIPSTERCall_constructor)).ToLocalChecked(), 1, new_call_args).ToLocalChecked();
        ...
        try {
          ...
          for (unsigned i = 0, m = 0; i < ci.media.size(); ++i) {
            if (ci.media[i].type == PJMEDIA_TYPE_AUDIO
              ...
              med_obj = Nan::NewInstance(Nan::GetFunction(Nan::New(SIPSTERMedia_constructor)).ToLocalChecked(), 0, NULL).ToLocalChecked(); 
...
static NAN_METHOD(CreatePlayer) {
  ...
  if (info.Length() > 0 && info[0]->IsString()) {
    ...
    src = string(*src_str);
    if (info.Length() > 1 && info[1]->IsBoolean() && Nan::To<bool>(info[1]).ToChecked())
...
static NAN_METHOD(CreatePlaylist) {
  ...
  if (info.Length() > 0 && info[0]->IsArray()) {
    ...
    for (uint32_t i = 0; i < arr_length; ++i) {
      Nan::Utf8String filename_str(Nan::Get(arr_obj, i).ToLocalChecked());
...
static NAN_METHOD(EPInit) {
  ...
  if (info.Length() > 0 && info[0]->IsObject()) {
    Local<Object> cfg_obj = Nan::To<v8::Object>(info[0]).ToLocalChecked();
    val = Nan::Get(cfg_obj, Nan::New("uaConfig").ToLocalChecked()).ToLocalChecked();
    if (val->IsObject()) {
      UaConfig uaConfig;
      Local<Object> ua_obj =  Nan::To<v8::Object>(val).ToLocalChecked();
      JS2PJ_UINT(ua_obj, maxCalls, uaConfig);
      JS2PJ_UINT(ua_obj, threadCnt, uaConfig);
      JS2PJ_BOOL(ua_obj, mainThreadOnly, uaConfig);

      val = Nan::Get(ua_obj, Nan::New("nameserver").ToLocalChecked()).ToLocalChecked();
      if (val->IsArray()) {
        ...
        if (arr_length > 0) {
          ...
          for (uint32_t i = 0; i < arr_length; ++i) {
            const Local<Value> arr_val = Nan::Get(arr_obj, i).ToLocalChecked();
            if (arr_val->IsString()) {
      ...
      val = Nan::Get(ua_obj, Nan::New("stunServer").ToLocalChecked()).ToLocalChecked();
      if (val->IsArray()) {
        const Local<Array> arr_obj = Local<Array>::Cast(val);
        const uint32_t arr_length = arr_obj->Length();
        if (arr_length > 0) {
    ...
    val = Nan::Get(cfg_obj, Nan::New("logConfig").ToLocalChecked()).ToLocalChecked();
    if (val->IsObject()) {
      LogConfig logConfig;
      Local<Object> log_obj = Nan::To<v8::Object>(val).ToLocalChecked();
      ...
      val = Nan::Get(log_obj, Nan::New("writer").ToLocalChecked()).ToLocalChecked();
      if (val->IsFunction()) {
        if (logger) {
          delete logger;
    ...
    val = Nan::Get(cfg_obj, Nan::New("medConfig").ToLocalChecked()).ToLocalChecked();
    if (val->IsObject()) {
      MediaConfig medConfig;
      Local<Object> med_obj = Nan::To<v8::Object>(val).ToLocalChecked();
  ...
  if ((info.Length() == 1 && info[0]->IsBoolean() && Nan::To<bool>(info[0]).ToChecked())
      || (info.Length() > 1
          && info[1]->IsBoolean()
...
#undef X
    ...
    Nan::Set(target,
             ...
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPVersion)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("state").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPGetState)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("config").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPGetConfig)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("init").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPInit)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("start").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPStart)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("hangupAllCalls").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPHangupAllCalls)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("mediaActivePorts").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPMediaActivePorts)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("mediaMaxPorts").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(EPMediaMaxPorts)).ToLocalChecked());

    Nan::Set(target,
             Nan::New("createRecorder").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(CreateRecorder)).ToLocalChecked());
    Nan::Set(target,
             Nan::New("createPlayer").ToLocalChecked(),
             Nan::GetFunction(Nan::New<FunctionTemplate>(CreatePlayer)).ToLocalChecked());
    Nan::Set(target,
