...
AccountConfig SIPSTERAccount::genConfig(Local<Object> acct_obj) {
  ...
  val = Nan::Get(acct_obj, Nan::New("regConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountRegConfig regConfig;
    Local<Object> reg_obj = Nan::To<v8::Object>(val).ToLocalChecked();
    JS2PJ_STR(reg_obj, registrarUri, regConfig);
    JS2PJ_BOOL(reg_obj, registerOnAdd, regConfig);

    val = Nan::Get(reg_obj, Nan::New("headers").ToLocalChecked()).ToLocalChecked();
    if (val->IsObject()) {
      const Local<Object> hdr_obj = Nan::To<v8::Object>(val).ToLocalChecked();
      const Local<Array> hdr_props = Nan::GetPropertyNames(hdr_obj).ToLocalChecked();
      const uint32_t hdr_length = hdr_props->Length();
      if (hdr_length > 0) {
        vector<SipHeader> sipheaders;
        for (uint32_t i = 0; i < hdr_length; ++i) {
          const Local<Value> key = Nan::Get(hdr_props, i).ToLocalChecked();
          const Local<Value> value = Nan::Get(hdr_obj, key).ToLocalChecked();
          SipHeader hdr;
  ...
  val = Nan::Get(acct_obj, Nan::New("sipConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountSipConfig sipConfig;
    Local<Object> sip_obj = Nan::To<v8::Object>(val).ToLocalChecked();

    val = Nan::Get(sip_obj, Nan::New("authCreds").ToLocalChecked()).ToLocalChecked();
    if (val->IsArray()) {
      ...
      if (arr_length > 0) {
        ...
        for (uint32_t i = 0; i < arr_length; ++i) {
          const Local<Value> cred_value = Nan::Get(arr_obj, i).ToLocalChecked();
          if (cred_value->IsObject()) {
            const Local<Object> auth_obj = Nan::To<v8::Object>(cred_value).ToLocalChecked();
            AuthCredInfo credinfo;
            Local<Value> scheme_val = Nan::Get(auth_obj, Nan::New("scheme").ToLocalChecked()).ToLocalChecked();
            Local<Value> realm_val = Nan::Get(auth_obj, Nan::New("realm").ToLocalChecked()).ToLocalChecked();
            Local<Value> username_val = Nan::Get(auth_obj, Nan::New("username").ToLocalChecked()).ToLocalChecked();
            Local<Value> dataType_val = Nan::Get(auth_obj, Nan::New("dataType").ToLocalChecked()).ToLocalChecked();
            Local<Value> data_val = Nan::Get(auth_obj, Nan::New("data").ToLocalChecked()).ToLocalChecked();
            if (scheme_val->IsString()
              ...
              credinfo.dataType = dataType_val->Int32Value(Nan::GetCurrentContext()).FromJust();
    ...
    val = Nan::Get(sip_obj, Nan::New("proxies").ToLocalChecked()).ToLocalChecked();
    if (val->IsArray()) {
      const Local<Array> arr_obj = Local<Array>::Cast(val);
      const uint32_t arr_length = arr_obj->Length();
      if (arr_length > 0) {
        ...
        for (uint32_t i = 0; i < arr_length; ++i) {
          const Local<Value> value = Nan::Get(arr_obj, i).ToLocalChecked();
    ...
    // JS land
    val = Nan::Get(sip_obj, Nan::New("transport").ToLocalChecked()).ToLocalChecked();
  ...
  val = Nan::Get(acct_obj, Nan::New("callConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountCallConfig callConfig;
    Local<Object> call_obj = Nan::To<v8::Object>(val).ToLocalChecked();
  ...
  val = Nan::Get(acct_obj, Nan::New("presConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountPresConfig presConfig;
    Local<Object> pres_obj = Nan::To<v8::Object>(val).ToLocalChecked();

    val = Nan::Get(pres_obj, Nan::New("headers").ToLocalChecked()).ToLocalChecked();
    if (val->IsObject()) {
      const Local<Object> hdr_obj = Nan::To<v8::Object>(val).ToLocalChecked();
      const Local<Array> hdr_props = Nan::GetPropertyNames(hdr_obj).ToLocalChecked();
      const uint32_t hdr_length = hdr_props->Length();
  ...
  val = Nan::Get(acct_obj, Nan::New("mwiConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountMwiConfig mwiConfig;
    Local<Object> mwi_obj = Nan::To<v8::Object>(val).ToLocalChecked();
    JS2PJ_BOOL(mwi_obj, enabled, mwiConfig);
    JS2PJ_UINT(mwi_obj, expirationSec, mwiConfig);

    acct_cfg.mwiConfig = mwiConfig;
  }
  val = Nan::Get(acct_obj, Nan::New("natConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountNatConfig natConfig;
    Local<Object> nat_obj = Nan::To<v8::Object>(val).ToLocalChecked();
  ...
  val = Nan::Get(acct_obj, Nan::New("mediaConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountMediaConfig mediaConfig;
    Local<Object> media_obj = Nan::To<v8::Object>(val).ToLocalChecked();

    val = Nan::Get(media_obj, Nan::New("transportConfig").ToLocalChecked()).ToLocalChecked();
    if (val->IsObject()) {
      TransportConfig transportConfig;
      Local<Object> obj = Nan::To<v8::Object>(val).ToLocalChecked();
      ...
      val = Nan::Get(obj, Nan::New("qosParams").ToLocalChecked()).ToLocalChecked();
      if (val->IsObject()) {
        pj_qos_params qos_params;
        Local<Object> qos_obj = Nan::To<v8::Object>(val).ToLocalChecked();
        Local<Value> flags_val = Nan::Get(qos_obj, Nan::New("flags").ToLocalChecked()).ToLocalChecked();
        Local<Value> dscp_val = Nan::Get(qos_obj, Nan::New("dscp_val").ToLocalChecked()).ToLocalChecked();
        Local<Value> so_prio_val = Nan::Get(qos_obj, Nan::New("so_prio").ToLocalChecked()).ToLocalChecked();
        Local<Value> wmm_prio_val = Nan::Get(qos_obj, Nan::New("wmm_prio").ToLocalChecked()).ToLocalChecked();
        if (flags_val->IsUint32()) {
          qos_params.flags =
            static_cast<pj_uint8_t>(flags_val->Uint32Value(Nan::GetCurrentContext()).FromJust());
        }
        if (dscp_val->IsUint32()) {
          qos_params.dscp_val =
            static_cast<pj_uint8_t>(dscp_val->Uint32Value(Nan::GetCurrentContext()).FromJust());
        }
        if (so_prio_val->IsUint32()) {
          qos_params.so_prio =
            static_cast<pj_uint8_t>(so_prio_val->Uint32Value(Nan::GetCurrentContext()).FromJust());
        }
        if (wmm_prio_val->IsUint32()) {
          qos_params.wmm_prio =
            static_cast<pj_qos_wmm_prio>(wmm_prio_val->Uint32Value(Nan::GetCurrentContext()).FromJust());
        }
        transportConfig.qosParams = qos_params;
      }

      val = Nan::Get(obj, Nan::New("tlsConfig").ToLocalChecked()).ToLocalChecked();
      if (val->IsObject()) {
        TlsConfig tlsConfig;
        Local<Object> tls_obj = Nan::To<v8::Object>(val).ToLocalChecked();
        ...
        val = Nan::Get(tls_obj, Nan::New("ciphers").ToLocalChecked()).ToLocalChecked();
        if (val->IsArray()) {
          const Local<Array> arr_obj = Local<Array>::Cast(val);
          const uint32_t arr_length = arr_obj->Length();
          if (arr_length > 0) {
            ...
            for (uint32_t i = 0; i < arr_length; ++i) {
              const Local<Value> value = Nan::Get(arr_obj, i).ToLocalChecked();
              ciphers.push_back(value->Int32Value(Nan::GetCurrentContext()).FromJust());
            }
        ...
        val = Nan::Get(tls_obj, Nan::New("qosParams").ToLocalChecked()).ToLocalChecked();
        if (val->IsObject()) {
          pj_qos_params qos_params;
          Local<Object> qos_obj = Nan::To<v8::Object>(val).ToLocalChecked();
          Local<Value> flags_val = Nan::Get(qos_obj, Nan::New("flags").ToLocalChecked()).ToLocalChecked();
  ...
  val = Nan::Get(acct_obj, Nan::New("videoConfig").ToLocalChecked()).ToLocalChecked();
  if (val->IsObject()) {
    AccountVideoConfig videoConfig;
    Local<Object> vid_obj = Nan::To<v8::Object>(val).ToLocalChecked();
...
NAN_METHOD(SIPSTERAccount::New) {
  ...
  if (info.Length() > 0 && info[0]->IsObject()) {
    acct_cfg = genConfig(Nan::To<v8::Object>(info[0]).ToLocalChecked());

    if (info.Length() > 1 && info[1]->IsBoolean())
      isDefault = Nan::To<bool>(info[1]).ToChecked();
  } else if (info.Length() > 0 && info[0]->IsBoolean())
    isDefault = Nan::To<bool>(info[0]).ToChecked();
  ...
  acct->emit = new Nan::Callback(
    Local<Function>::Cast(Nan::Get(acct->handle(), Nan::New(emit_symbol)).ToLocalChecked())
  );
...
NAN_SETTER(SIPSTERAccount::DefaultSetter) {
  ...
  if (Nan::To<bool>(value).ToChecked()) {
    try {
      acct->setDefault();
    } catch(Error& err) {
...
NAN_METHOD(SIPSTERAccount::SetRegistration) {
  ...
  if (info.Length() > 0 && info[0]->IsBoolean())
    renew = Nan::To<bool>(info[0]).ToChecked();
  else
    return Nan::ThrowTypeError("Missing renew argument");
...
NAN_METHOD(SIPSTERAccount::MakeCall) {
  ...
  if (info.Length() > 0 && info[0]->IsString()) {
    ...
    if (info.Length() > 1) {
      prm.statusCode = static_cast<pjsip_status_code>(info[1]->Int32Value(Nan::GetCurrentContext()).FromJust());
  ...
  Local<Object> call_obj =
    Nan::NewInstance(Nan::GetFunction(Nan::New(SIPSTERCall_constructor)).ToLocalChecked(), 1, new_call_args).ToLocalChecked();
