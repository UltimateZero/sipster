  ...
  if (info.Length() > 0 && info[0]->IsObject()) {
    ...
    if (val->IsObject()) {
      ...
      if (flags_val->IsUint32())
        qos_params.flags = static_cast<pj_uint8_t>(flags_val->Uint32Value(Nan::GetCurrentContext()).FromJust());
...
# endif
  ...
  trans->emit = new Nan::Callback(
    Local<Function>::Cast(Nan::Get(trans->handle(), Nan::New(emit_symbol)).ToLocalChecked())
  );
